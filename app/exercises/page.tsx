...
function ExerciseCard({ exercise, isCustom }: { exercise: Exercise; isCustom: boolean }) {
  return (
    <Link href={`/exercises/${exercise.id}`}>
      <Card className="h-full hover:shadow-md transition-shadow cursor-pointer">
        <CardHeader className="pb-3">
          ...
          {exercise.equipment && <p className="text-sm text-gray-600">{exercise.equipment}</p>}
        </CardHeader>
        <CardContent className="pt-0">
          {exercise.description && <p className="text-sm text-gray-700 mb-3 line-clamp-2">{exercise.description}</p>}
